...
def machine_instance(request, pk):
    ...
    # Import docker_manager with error handling
    docker_manager = None
    try:
        from docker_service import docker_manager
    except ImportError as e:
        return Response({
            'error': f'Docker service import failed: {str(e)}. Make sure docker_service.py exists in backend folder.'
        }, status=500)
    except Exception as e:
        return Response({'error': f'Docker service error: {str(e)}'}, status=500)
...
def certificate_eligibility(request):
    ...
    eligibility['is_admin'] = request.user.is_superuser
