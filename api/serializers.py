...
from .models import UserProfile, Room, RoomTask, Machine, RoomMembership, TaskSubmission, MachineSubmission, MachineRating, Frame


class UserProfileSerializer(serializers.ModelSerializer):
    class Meta:
        model = UserProfile
        fields = ['rank', 'points', 'avatar', 'frame', 'unlocked_frames', 'completed_machines']
...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def validate_username(self, value):
        if User.objects.filter(username=value).exists():
            raise serializers.ValidationError(f"{value} this name already used")
        return value
...
class FrameSerializer(serializers.ModelSerializer):
    """Serializer for profile frames"""
    class Meta:
        model = Frame
        fields = ['frame_id', 'name', 'description', 'required_points', 'border_color', 'gradient_start', 'gradient_end', 'icon']
