...
class UserProfile(models.Model):
    ...
    frame = models.CharField(max_length=50, default='default')  # Selected frame ID
    unlocked_frames = models.JSONField(default=list)  # List of unlocked frame IDs
    completed_machines = models.JSONField(default=list)  # List of machine IDs marked as completed
    completed_tasks = models.JSONField(default=dict)  # Dict of {machine_id: [task_indices]}
    ...
    def save(self, *args, **kwargs):
        ...
        # Auto-unlock frames based on points
        frame_thresholds = [50, 200, 400, 600, 800, 1000]
        frame_ids = ['bronze', 'silver', 'gold', 'platinum', 'diamond', 'conqueror']
        
        # Ensure unlocked_frames is a list
        if not isinstance(self.unlocked_frames, list):
            self.unlocked_frames = []
        
        # Unlock frames based on points
        for i, threshold in enumerate(frame_thresholds):
            if self.points >= threshold and frame_ids[i] not in self.unlocked_frames:
                self.unlocked_frames.append(frame_ids[i])
...
class Frame(models.Model):
    """Profile frames that can be unlocked with points - PUBG conquer style"""
    FRAME_TYPES = [
        ('bronze', 'Bronze'),
        ('silver', 'Silver'),
        ('gold', 'Gold'),
    ...
    frame_id = models.CharField(max_length=20, unique=True)  # bronze, silver, etc.
    name = models.CharField(max_length=50, unique=True)
    description = models.CharField(max_length=200)
    required_points = models.IntegerField()
    border_color = models.CharField(max_length=20, default='#cd7f32')  # Hex color for frame
    gradient_start = models.CharField(max_length=20, default='#cd7f32')
    gradient_end = models.CharField(max_length=20, default='#8b4513')
...
class VPNConfig(models.Model):
    ...
    name = models.CharField(max_length=100, default='CyberTraining VPN', unique=True)
...
class Certificate(models.Model):
    """Certificate for users who complete all machines"""
